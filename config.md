...
结束时间的记录一次性发送给服务器 Agent，写入数据库。**
5. 如果系统重启或异常导致没有捕捉到结束在树莓派的串口外接相机上，捕获到人脸快照（以后可能会进行活体检测），并将快照通过接口上传到服务器进行人脸识别。
...
4. **仅在此刻，将包含完整开始时间、时间，则该条记录作废，不予上报（因为没有完整的时间段信息）。
...
仓库摄像头视频流：rtsp://user:password@camera-host:554/Streaming/Channels/101
服务器人脸识别接口：http://your-agent-host/api/system/visitorRecord/recognizeFace
